...
testfile = args[1]
test_groupfile = args[4]

name = paste0(group1, "_vs_", group2, "_RF_")
...
testgroup <- read.table(test_groupfile,row.names = 1,stringsAsFactors = F)
...
afs = fselect(method = "sequential",task = fs.task,
              ...
              terminator = trm("stagnation", iters = 40, 
                               threshold = 0))
afs$result
...
#feature importance
feature_import = flt("importance", learner = classif.lrn)
feature_import$calculate(fs.task)
feature_import = as.data.table(feature_import)

p4 <- ggbarplot(feature_import, 
                x = "feature", 
                y = "score",
                color = "#0073C2FF",
                fill = "#0073C2FF",
...
write.table(feature_import, paste0(name,'_importance.txt'), sep = '\t', col.names = NA, quote = FALSE)
ggsave(paste0(name, "_feature_importance.png"), p4, width = 6, height = 4)
ggsave(paste0(name, "_feature_importance.pdf"), p4, width = 6, height = 4)
